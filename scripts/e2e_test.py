...
def run():
    ...
    r = requests.post(
        f"{BASE}/api/v1/users/",
        json={
            "username": "e2e_runner2",
            "email": "e2e_runner2@example.com",
            "password": "runner123",
        },
    )
    ...
    r = requests.post(
        ...
        data={"username": "e2e_runner2@example.com", "password": "runner123"},
    )
    ...
    r = requests.post(
        f"{BASE}/api/v1/tasks/",
        headers=headers,
        json={"title": "复习 FastAPI", "description": "阅读核心源码", "status": "pending"},
    )
    task = r.json()
    ...
    r = requests.put(
        ...
        headers=headers,
        json={"status": "completed", "description": "已完成源码阅读"},
    )
