...
def run():
    ...
    # 7. Update task
    r = requests.put(f"{BASE}/api/tasks/{task['id']}", headers=headers, json={
        "status": "completed", "description": "已完成源码阅读"
    })
    print(f"7. Updated: status={r.json()['status']}")

    # 8. Delete task
    r = requests.delete(f"{BASE}/api/tasks/{task['id']}", headers=headers)
    print(f"8. Deleted: {r.status_code}")

    # 9. Verify deletion
    r = requests.get(f"{BASE}/api/tasks/{task['id']}", headers=headers)
