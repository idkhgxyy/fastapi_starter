...
r = requests.post(f"{BASE}/api/users/", json={
    "username": "rag_tester", "email": email, "password": password
})
...
with open("sample_knowledge.txt", "rb") as f:
    r = requests.post(f"{BASE}/api/rag/upload", headers=headers,
                      files={"file": ("sample.txt", f, "text/plain")})
print(f"3. Upload .txt: {r.status_code}, file_type={r.json().get('file_type')}, status={r.json().get('status')}")

# Upload .md
with open("sample_knowledge.md", "rb") as f:
    r = requests.post(f"{BASE}/api/rag/upload", headers=headers,
                      files={"file": ("sample.md", f, "text/markdown")})
print(f"4. Upload .md:  {r.status_code}, file_type={r.json().get('file_type')}, status={r.json().get('status')}")

# Upload .pdf
with open("sample_knowledge.pdf", "rb") as f:
    r = requests.post(f"{BASE}/api/rag/upload", headers=headers,
                      files={"file": ("sample.pdf", f, "application/pdf")})
print(f"5. Upload .pdf: {r.status_code}, file_type={r.json().get('file_type')}, status={r.json().get('status')}")
...
# List documents
r = requests.get(f"{BASE}/api/rag/documents", headers=headers)
docs = r.json()
type_counts = {}
for d in docs:
